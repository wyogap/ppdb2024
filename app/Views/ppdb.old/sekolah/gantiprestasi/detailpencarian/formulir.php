...
<div class="row">
	<?php 
		...
		$skoring_pendaftaran_id = 0;
		$daftar_nilai_skoring_id = 0;
		// foreach($detailnilaiprestasi->getResult() as $row):
		// 	$skoring_pendaftaran_id = $row->skoring_pendaftaran_id;
		// 	$daftar_nilai_skoring_id = $row->daftar_nilai_skoring_id;
		// endforeach;
		foreach($profilsiswa->getResult() as $row):
			...
			$lintang = $row->lintang;
			$bujur = $row->bujur;
			$daftar_nilai_skoring_id = $row->prestasi_skoring_id;
			$skoring_pendaftaran_id = $row->skor_prestasi;
	?>
	...
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		<div class="box box-solid">
			...
			<div class="box-footer">
				<div class="row">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<form role="form" enctype="multipart/form-data" id="prosesubahprestasi" action="<?php echo base_url();?>index.php/Csekolah/prosesubahprestasi/" method="post">
							<input type="hidden" id="pendaftaran_id" name="pendaftaran_id" value="<?php echo $this->input->get("pendaftaran_id");?>">
							<input type="hidden" id="penerapan_id" name="penerapan_id" value="<?php echo $this->input->get("penerapan_id");?>">
							<div class="form-group has-feedback">
								<label for="daftar_nilai_skoring_id">Jenis Prestasi</label>
								<select id="daftar_nilai_skoring_id" name="daftar_nilai_skoring_id" class="form-control select2" data-validation="required">
									<option value="">-- Jenis Prestasi --</option>
									<?php foreach($daftarnilaiskoringprestasi->getResult() as $row3):?>
									<option <?php if($daftar_nilai_skoring_id==$row3->daftar_nilai_skoring_id){?>selected="true"<?php }?> value="<?php echo $row3->daftar_nilai_skoring_id;?>"><?php echo $row3->keterangan;?> - <?php echo $row3->jenis;?> (<?php echo $row3->nilai;?>)</option>
									<?php endforeach;?>
								</select>
							</div>
							<button type="submit" class="btn btn-primary btn-flat">Update Prestasi</button>
						</form>
					</div>
				</div>
			</div>
