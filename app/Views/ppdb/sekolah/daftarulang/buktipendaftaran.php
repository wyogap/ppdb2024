<html>
	...
	<body>
		...
		<div align="center">
			<b>TANDA BUKTI PENDAFTARAN<br>PENERIMAAN PESERTA DIDIK BARU (PPDB) ONLINE <?php echo strtoupper($wilayah_aktif);?><br>TAHUN <?php echo $tahun_ajaran_aktif; ?></b>
		</div><br>
		<p>Identitas siswa :</p>
		<table width="100%">
		...
		<table width="100%" style="border-collapse: collapse;" border=1>
			...
			<tr>
				...
				<?php if($row->status_penerimaan==0){?>Dalam Proses Seleksi
				<?php }else if($row->status_penerimaan==1){?>Diterima
				<?php }else if($row->status_penerimaan==2){?>Tidak Diterima
				<!-- Pada saat daftar ulang Daftar Tunggu == Masuk Kuota !-->
				<?php }else if($row->status_penerimaan==3){?>Diterima
				<?php }else if($row->status_penerimaan==4){?>Diterima Pilihan <?php echo $row->masuk_jenis_pilihan ?>
		...
		<table width="100%">
			...
			<?php foreach($profilsiswa->getResult() as $row):?>
			<tr>
				<td valign="top" width="25%">Lokasi Berkas</td>
				<td valign="top">:</td>
