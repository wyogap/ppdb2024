...
Class Mpengajuanakun 
{
	...
	function daftarpengajuanakun(){
		...
		$builder->select('a.pengguna_id,d.npsn,d.sekolah_id,d.nama AS sekolah,b.nisn,a.nama,c.nama_kab AS kabupaten,a.user_name as username');
		$builder->join('tcg_peserta_didik b','a.peserta_didik_id = b.peserta_didik_id AND b.is_deleted = 0');
		$builder->join('ref_wilayah c','b.kode_wilayah = c.kode_wilayah AND c.expired_date IS NULL','LEFT OUTER');
	...
	function daftarpersetujuanakun(){
		$builder = $this->db->table('dbo_users a');
		$builder->select('a.pengguna_id,d.npsn,d.sekolah_id,d.nama AS sekolah,b.nisn,a.nama,c2.nama AS kabupaten,a.user_name as username,a.approval');
		$builder->join('tcg_peserta_didik b','a.peserta_didik_id = b.peserta_didik_id AND b.is_deleted = 0');
		$builder->join('ref_wilayah c','b.kode_wilayah = c.kode_wilayah AND c.expired_date IS NULL','LEFT OUTER');
	...
	function daftartidaksetujuakun(){
		$builder = $this->db->table('dbo_users a');
		$builder->select('a.pengguna_id,d.npsn,d.sekolah_id,d.nama AS sekolah,b.nisn,a.nama,c.nama_kab AS kabupaten,a.user_name as username,a.approval');
		$builder->join('tcg_peserta_didik b','a.peserta_didik_id = b.peserta_didik_id AND b.is_deleted = 0');
		$builder->join('ref_wilayah c','b.kode_wilayah = c.kode_wilayah AND c.expired_date IS NULL','LEFT OUTER');
		$builder->join('ref_sekolah d','b.sekolah_id = d.sekolah_id','LEFT OUTER');
	...
	function detailpengajuanakun($pengguna_id){
		...
		$builder->where(array('a.pengguna_id'=>$pengguna_id,'a.approval'=>0,'a.role_id'=>1,'a.registrasi'=>1,'a.is_deleted'=>0));
		return $builder->get();
	}
