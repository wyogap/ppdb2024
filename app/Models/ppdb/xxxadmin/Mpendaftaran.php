...
Class Mpendaftaran 
{
	...
	function tcg_resetpendaftaran($peserta_didik_id) {
		...
		$query = "CALL " .SQL_RESET_PENDAFTARANSISWA. "('$peserta_didik_id', '$user_id')";
		$this->db->query($query);
	...
	function tcg_cari_pendaftaran($nama, $nisn, $nik, $sekolah_id, $jenis_kelamin, $kode_desa, $kode_kecamatan, $sekolah_tujuan_id, $penerapan_id, $is_deleted){
		...
		$query = "select a.peserta_didik_id, a.nama, a.nisn, a.nik, c.nama as sekolah_asal, e.nama as sekolah_tujuan, f.nama as penerapan, z.*
				  ...
				  join dbo_users d on d.peserta_didik_id=a.peserta_didik_id and d.is_deleted=0
	...
	// 	return $this->db->query("CALL " .SQL_RESET_PENDAFTARANSISWA. " ('$peserta_didik_id', '$user_id')");
	// }
