...
Class Mprofilsekolah 
{
    ...
    function tcg_daftarpenerapan($sekolah_id){
        ...
        select a.penerapan_id,c.jalur_id,a.nama AS jalur,a.tooltip,d.kuota, coalesce(e.tambahan_kuota,0) as tambahan_kuota, 
            ...
            a.jenjang_id, a.kategori_inklusi
        ...
        join ref_sekolah f on f.sekolah_id=d.sekolah_id
        left join (
        ...
        where a.aktif=1 and a.is_deleted=0 and a.perankingan=1 and d.sekolah_id=? and a.tahun_ajaran_id=? and a.putaran=?
            and (a.kategori_inklusi=0 or f.inklusi=1)
        order by a.urutan";
        
        // $arr = array($sekolah_id, TAHUN_AJARAN_ID, $putaran, $sekolah_id, TAHUN_AJARAN_ID, $putaran);
        // var_dump($arr); exit;

		$result = $this->ro->query($query, array($sekolah_id, TAHUN_AJARAN_ID, $putaran, $sekolah_id, TAHUN_AJARAN_ID, $putaran))->getResultArray();
    ...
    function tcg_infopenerapan($sekolah_id, $penerapan_id){
		$putaran = $this->session->get('putaran_aktif');
        ...
        left join (
            select penerapan_id, count(*) as cnt_pendaftaran from tcg_pendaftaran 
            where is_deleted=0 and cabut_berkas=0 and sekolah_id=? and tahun_ajaran_id=? and putaran=? and penerapan_id=?
            group by penerapan_id
        ) g on g.penerapan_id=a.penerapan_id
	...
	function tcg_rekappendaftar($sekolah_id){
		$builder = $this->ro->table('tcg_pendaftaran a');
		$builder->select('a.pendaftaran_id,a.sekolah_id,a.peserta_didik_id,a.penerapan_id,a.nomor_pendaftaran,a.kelengkapan_berkas,a.created_on');
        ...
        $builder->select('e.nama AS sekolah_asal,f.nama AS lokasi_berkas,g.keterangan as label_masuk_pilihan,h.keterangan as label_jenis_pilihan,i.nama as sedang_verifikasi');
        $builder->select('c.nama as penerapan, d.jalur_id, d.nama AS jalur, b.nomor_kontak');
        $builder->select('case when a.status_daftar_ulang = 1 then a.tanggal_daftar_ulang else NULL end as tanggal_daftar_ulang', false);
        $builder->select('a.skor_jarak, b.nilai_mat, b.nilai_bin');
        $builder->select('x1.nama as prestasi_kejuaraan, x2.nama as prestasi_akademik, x3.nama as prestasi_organisasi');
        $builder->select("case when a.status_penerimaan_final in (1,3) then 'Diterima' else 'Tidak Diterima' end as status_penerimaan_label", false);
		$builder->join('tcg_peserta_didik b','a.peserta_didik_id = b.peserta_didik_id AND b.is_deleted = 0');
		$builder->join('cfg_penerapan c','a.penerapan_id = c.penerapan_id AND c.aktif = 1 and c.tahun_ajaran_id=a.tahun_ajaran_id and c.putaran=a.putaran AND c.is_deleted=0','LEFT OUTER');
		$builder->join('ref_jalur d','c.jalur_id = d.jalur_id AND d.is_deleted=0','LEFT OUTER');
		$builder->join('ref_sekolah e','b.sekolah_id = e.sekolah_id','LEFT OUTER');
        ...
        $builder->join('cfg_daftar_skoring x1','x1.skoring_id = a.prestasi_skoring_id and x1.is_deleted = 0','LEFT OUTER');		
        $builder->join('cfg_daftar_skoring x2','x2.skoring_id = a.akademik_skoring_id and x2.is_deleted = 0','LEFT OUTER');		
        $builder->join('cfg_daftar_skoring x3','x3.skoring_id = a.organisasi_skoring_id and x3.is_deleted = 0','LEFT OUTER');		
        $builder->where(array('a.cabut_berkas'=>0,'a.jenis_pilihan !='=>0,'a.is_deleted'=>0));
        $builder->where('a.tahun_ajaran_id', TAHUN_AJARAN_ID);
        ...
        if (empty($orderby)) {
            $builder->orderBy('a.created_on, a.pendaftaran_id');
        }
        else {
            $builder->orderBy($orderby);
        }
	...
	function tcg_penerimaan_sd($sekolah_id, $penerapan_id, $filter=null) {
        ...
        $builder->select("a.status_daftar_ulang");
