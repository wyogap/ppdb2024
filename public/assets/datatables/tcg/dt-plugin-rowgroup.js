...
/**
 * @summary     CollapsibleRowGroup
 * @description Collapsible RowGrouping for DataTables
 * @version     1.1.1
 * @file        dt-plugin-collapsibleRowGroup.js
 * @author      Wahyu Yoga Pratama
 * @contact     wyogap@gmail.com
...
var CollapsibleRowGroup = function ( dt, opts ) {
	// Sanity check that we are using DataTables 1.10 or newer
	if ( ! DataTable.versionCheck || ! DataTable.versionCheck( '1.10.8' ) ) {
		throw 'CollapsibleRowGroup requires DataTables 1.10.8 or newer';
	}

	// User and defaults configuration object
	this.c = $.extend( true, {},
		DataTable.defaults.collapsibleRowGroup,
		CollapsibleRowGroup.defaults,
		opts
	);
	...
	this.dom = {
		filterContainer: $('<div/>')
			.addClass( "dataTables_filter" ).addClass( "dt-collapsibleRowGroup-filter" ).attr("id", instance+"_filter")
			.append( $( '<label>' )
				.html( strSearch )
				.append( $( '<input>' )
					.attr('type', 'search')
	...
	var existing = settings.collapsibleRowGroup;
	if ( existing ) {
		return existing;
	}

	settings.collapsibleRowGroup = this;
	this._constructor();
};


$.extend( CollapsibleRowGroup.prototype, {
	/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	 ...
	 * @returns string~CollapsibleRowGroup
	 */
	...
	_constructor: function ()
	{
		...
		dtPrivateSettings.oApi._fnCallbackReg( dtPrivateSettings, 'aoRowCreatedCallback', function (tr, data, idx) {
			...
			//let init complete first
			if (that.s.groups == null) {
				//after ajax.reload. no need to do it one by one. do it for all.
				return;
			}

			//in the middle of reloading, do it for all instead of one-by-one
			if (that.s.isReloading) {
				return;
			}
		...
		dt.on( 'init.dt', function () {
			...
			if (that.s.debug)
				console.log( dt.context[0].sInstance+ ': Initialisation start: '+new Date().getTime() );

			if (that.s.groups == null) {
				var rows = dt.rows();
				var groups = that._group( 0, rows.indexes(), 0 );
				...
				// 		if (typeof settings.collapsibleRowGroup === 'undefined' || settings.collapsibleRowGroup == null) {
				// 			return true;
				// 		}
				// 		return settings.collapsibleRowGroup.s.filteredRows == null || settings.collapsibleRowGroup.s.filteredRows.includes(row[settings.rowId]);
				// 	}
				// );
				// $.fn.dataTable.ext.search.push(
				// 	function(settings, data, dataIndex, row) {
				...
				// 			return true;
				// 		}
				// 		return !settings.collapsibleRowGroup.s.collapsedRows.includes(row[settings.rowId]);
				// 	}
				// );
			...
			if (that.s.debug)
				console.log( dt.context[0].sInstance+ ': Initialisation complete: '+new Date().getTime() );

			dtPrivateSettings.rowgroup = that;
		});
	
		//Important: when the first ajax load result in empty data, the init.dt event is never called.
		//Thus, we have to use combination of xhr.dt and preDraw.dt event to trigger rowgroup initialization on ajax reload
		dt.on( 'xhr.dt', function () {
			var dt = that.s.dt;
			
			that.s.isReloading = true;
		});

		dt.on( 'preDraw.dt', function () {
			var dt = that.s.dt;
			...
			if (that.s.isReloading) {
				...
				that._restoreCollapsible(that.s.groups, groups);
				that.s.groups = groups;
				...
				if (that.s.debug)
					console.log( dt.context[0].sInstance+ ': Load data complete: '+new Date().getTime() );

				dtPrivateSettings.rowgroup = that;

				that.s.isReloading = false;
			}
		});

		// Redraw the details box on each draw which will happen if the data has changed. 
		// This is used until DataTables implements a native `updated` event for rows
		dt.on( 'draw.dtr', function () {
			...
			//var rows = dt.rows();
			var groups = that._group( 0, indexes, 0 );
		...
		$.fn.dataTable.ext.search.push(
			function(settings, data, dataIndex, row) {
				...
				return settings.collapsibleRowGroup.s.filteredRows == null || settings.collapsibleRowGroup.s.filteredRows.length == 0 
						|| settings.collapsibleRowGroup.s.filteredRows.includes(row[settings.rowId]);
			}
		);

		that.s.collapsedRows = [];			
		$.fn.dataTable.ext.search.push(
			function(settings, data, dataIndex, row) {
				//return !that.s.collapsedRows.includes(dataIndex);
				if (typeof settings.collapsibleRowGroup === 'undefined' || settings.collapsibleRowGroup == null) {
	...
	_restoreCollapsible: function(oldGroups, newGroups) {
		if (typeof oldGroups === 'undefined' || oldGroups == null)
			return;

		let that = this;

		let grp = null;
		oldGroups.forEach(function(item, idx) {
			grp = that._getGroup(newGroups, item.dataPoint);
			if (grp == null) {
				return;
			}

			//restore
			grp.collapsed = item.collapsed;
			
			if (typeof item.children !== 'undefined' && item.children != null && item.children.length > 0) {
				that._restoreCollapsible(item.children, grp.children);
			}
		});
	...
	_insertRow: function ( tr, data, idx, groups, level ) {
		...
		//TODO: the proper way to insert a row is to actually iterate each level of grouping (from level 0 to 5)
		//      and find the corresponding group in global group list

		group = this._getGroup(groups, groupName);
		if (group == null) {
			group = {
						dataPoint: this._getGroupName(level, row),
						level: level,
	...
	_filter: function (forced) {
		...
		var filteredData = this.s.dt.rows().eq( 0 ).filter( function (rowIdx) {
			let row = that.s.dt.row(rowIdx);
			let obj = row.data();
		...
		for ( var i=0, ien=filteredData.length ; i<ien ; i++ ) {
			rowIndex = filteredData[i];
	...
	_filteredRow: function(groups, row, level) {
		...
		//include the group if any
		let groupName = this._getGroupName(level, row);
	...
	_displayRow: function ( row, level ) {
		let data = row.data();
		let cell = $(this.c.selector, $(row.node()));
		let tr = cell.closest('tr');

		//clean up tr
		tr.removeClass( 'dtrg-level-0' );
		tr.removeClass( 'dtrg-level-1' );
		tr.removeClass( 'dtrg-level-2' );
		tr.removeClass( 'dtrg-level-3' );
		tr.removeClass( 'dtrg-level-4' );

		//add the level for tr
		tr.addClass( 'dtrg-level-' + (level+1) );

		// let groupName = this._getGroupName(level, row);
		// tr.attr('dtrg-group-name', groupName);

		//remove lower level in case it is being moved, it will be added again when processing the children
		if (level <= 3) {
			cell.removeClass( 'dtrg-selector-level-4' );
		}
		if (level <= 2) {
			cell.removeClass( 'dtrg-selector-level-3' );
		}
		if (level <= 1) {
			cell.removeClass( 'dtrg-selector-level-2' );
		}
		if (level <= 0) {
			cell.removeClass( 'dtrg-selector-level-1' );
		}

		//add missing higher level in case it is being moved
		if (level > 0) {
			cell.addClass( 'dtrg-selector-level-0' );
		}
		if (level > 1) {
			cell.addClass( 'dtrg-selector-level-1' );
		}
		if (level > 2) {
			cell.addClass( 'dtrg-selector-level-2' );
		}
		if (level > 3) {
			cell.addClass( 'dtrg-selector-level-3' );
		}
	...
	_displayHeader: function ( group ) {
		...
		let level = group.effectiveLevel;
		...
		if (group.headerRow != null) {
			...
			node.removeClass( 'dtrg-level-0' );
			node.removeClass( 'dtrg-level-1' );
			node.removeClass( 'dtrg-level-2' );
			node.removeClass( 'dtrg-level-3' );
			node.removeClass( 'dtrg-level-4' );

			node.addClass( 'dtrg-level-'+level );
	...
	_collapsedGroups: function(groups) {
		...
		groups.forEach(function(item, idx) {
			if (item.collapsed == 1) {
				...
				//nothing since filters is reset in each call
			}	
	...
	_getGroupName: function(level, row) {
		let fns = $.isArray( this.c.dataSrc ) ? this.c.dataSrc : [ this.c.dataSrc ];
		let rowData = row.data();
		...
		for(var i=0; i<=level; i++) {
			let fn = DataTable.ext.oApi._fnGetObjectDataFn( fns[ i ] );
			let str = fn( rowData );
	 ...
	 * @private
	 */
	_group: function ( level, rows, effectiveLevel ) {
		let fns = $.isArray( this.c.dataSrc ) ? this.c.dataSrc : [ this.c.dataSrc ];
		let fn = DataTable.ext.oApi._fnGetObjectDataFn( fns[ level ] );
		let dt = this.s.dt;
		let groupName, last;
		...
		//ignore level if all data is null or empty
		...
		if (ignoreLevel) {
			if ( fns[ level+1 ] !== undefined) {
				return this._group( level+1, rows, effectiveLevel );
			}
			else {
				return [];
			}
		...
		if ( fns[ level+1 ] !== undefined && data.length>0) {
			...
			for ( let i=0; i<ien ; i++ ) {
				...
				data[i].children = this._group( level+1, that._getRowIndexes(dt,data[i].rows), effectiveLevel+1 );
			}
		}
...
/**
 * CollapsibleRowGroup default settings for initialisation
 *
 * @namespace
 * @name CollapsibleRowGroup.defaults
 * @static
 */
CollapsibleRowGroup.defaults = {
	/**
...
CollapsibleRowGroup.version = "0.0.1";


$.fn.dataTable.CollapsibleRowGroup = CollapsibleRowGroup;
$.fn.DataTable.CollapsibleRowGroup = CollapsibleRowGroup;


DataTable.Api.register( 'collapsibleRowGroup()', function () {
	return this;
} );

DataTable.Api.register( 'collapsibleRowGroup().disable()', function () {
	return this.iterator( 'table', function (ctx) {
		if ( ctx.collapsibleRowGroup ) {
			ctx.collapsibleRowGroup.enable( false );
		}
	} );
} );

DataTable.Api.register( 'collapsibleRowGroup().enable()', function ( opts ) {
	return this.iterator( 'table', function (ctx) {
		if ( ctx.collapsibleRowGroup ) {
			ctx.collapsibleRowGroup.enable( opts === undefined ? true : opts );
		}
	} );
} );

DataTable.Api.register( 'collapsibleRowGroup().dataSrc()', function ( val ) {
	if ( val === undefined ) {
		return this.context[0].collapsibleRowGroup.dataSrc();
	}

	return this.iterator( 'table', function (ctx) {
		if ( ctx.collapsibleRowGroup ) {
			ctx.collapsibleRowGroup.dataSrc( val );
		}
	} );
} );
...
$(document).on( 'preInit.dt.dtrg', function (e, settings, json) {
	...
	var init = settings.oInit.collapsibleRowGroup;
	var defaults = DataTable.defaults.collapsibleRowGroup;

	if ( init || defaults ) {
		var opts = $.extend( {}, defaults, init );

		if ( init !== false ) {
			new CollapsibleRowGroup( settings, opts  );
		}
	}
} );
...
function _initFilter ( settings ) {
	...
	var opts = api.init().collapsibleRowGroup || DataTable.defaults.collapsibleRowGroup;

	return new CollapsibleRowGroup( api, opts ).filterContainer();
}
...
if ( DataTable.ext.features ) {
	DataTable.ext.features.register( 'collapsibleRowGroup', _initFilter );
}

return CollapsibleRowGroup;
